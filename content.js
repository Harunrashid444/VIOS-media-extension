...
(function () {
  ...
  function buildOverlay() {
    ...
    // ----- Fullscreen -----
    //
    // In fullscreen the browser renders only the fullscreen element's subtree,
    // so an overlay parked on <body> simply vanishes (e.g. YouTube fullscreen).
    // Follow the fullscreen element in and back out.  A <video> or <iframe>
    // can't host visible children, so in those cases there is nothing to do.
    document.addEventListener('fullscreenchange', () => {
      const fsEl = document.fullscreenElement;
      if (fsEl && fsEl.tagName !== 'VIDEO' && fsEl.tagName !== 'IFRAME') {
    ...
    // Keep interactions with the panel to ourselves.  Once it lives inside a
    // video player, a bubbling click would toggle play/pause and a double-click
    // would exit fullscreen.
