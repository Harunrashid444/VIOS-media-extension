...
(function () {
  ...
  /** Media whose rate VIOS has actually changed (so a reset can hand it back). */
  const touchedMedia = new WeakSet();
   ...
   *
   * At 1× VIOS is idle and must keep its hands off: this runs on every site,
   * and enforcing 1× there overrode the page's own speed controls (YouTube's
   * speed menu and `<` / `>` shortcuts silently snapped back to 1×).  So the
   * lock only engages while a non-1 speed is selected, and on reset we hand
   * back only the elements we actually changed.
   */
  function lockMediaRate(el) {
    if (!knownMedia.has(el)) {
      knownMedia.add(el);

      // Re-apply if the courseware resets it — but only while VIOS is active.
      el.addEventListener('ratechange', () => {
        if (SPEED === 1) return; // idle: the page owns playbackRate
    ...
    if (SPEED === 1) {
      if (touchedMedia.has(el)) {
        touchedMedia.delete(el);
        try {
    ...
    try {
      ...
      // Unsupported / detached media – nothing we can do.
    }
  }
  ...
  _setInterval(() => {
    scanAndObserve(document);
    // Idle at 1×: leave the page's own animation rates alone.  (A reset to
    // 1× still restores them once, from the speed-change handler.)
    if (SPEED !== 1) scaleWebAnimations();
  }, 2000);
